...
function sendFile(ImageURL) {	
		...
		//var fd = new FormData();
		//fd.append('encodestring',ImageURL);
...
function performClick(elemId) {
 ...
 $(document).ready(function(){
 		var data;
 		$(document).on('change', 'input#file_attach', function(e) {
		//$("#uploaddiamond").on("submit", function(e) {
		    e.preventDefault();		   
		    var extension = $('#file_attach').val().split('.').pop().toLowerCase();
		    if ($.inArray(extension, ['csv','docx','doc','xls', 'xlsx','jpg','jpeg','png','pdf','zip','rar']) == -1) {
		        $('#errormessage').html('Please Select Valid File... ');
		    } else {
		    	var pr =$(this).parent();
		    	var file_data = $('#file_attach').prop('files')[0];
		    	//var file = document.querySelector('#files > input[type="file"]').files[0];
					// getBase64(file_data).then(
					//   data => console.log(data)
					// );
				//data = getBase64(file_data);
		        ...
		        $.ajax({
		            url: "/automark/admin/uploadfile", // point to server-side PHP script
		            data: form_data,
		            type: 'POST',
