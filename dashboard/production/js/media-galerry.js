...
_url_host = extractHostname(_url_host);
if(_url_host=='localhost'){
	var path = window.location.pathname.split('/');
	_url_host = window.location.protocol + "//" + window.location.host + "/" + path[1];
}else{
	_url_host = window.location.protocol + "//" + window.location.host;
}
...
function upload_files(){
	  var _csrf_token = document.getElementsByName("csrf-token")[0].getAttribute("content");
	  var http = new XMLHttpRequest();
	  var url = _url_host+"/admin/files/uploadfile";
	  var params = JSON.stringify({"file":ImageURL});
	  http.open("POST", url, true);
	  http.setRequestHeader("X-CSRF-TOKEN", _csrf_token);
	  http.setRequestHeader("Accept", "application/json");
