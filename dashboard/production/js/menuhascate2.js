...
function AddCateToMenu(){
  ...
  for (var i = 0;i <= _e_array_check.length - 1; i++) {
    if(_e_array_check[i].checked){
        ...
        var _e_menu = document.getElementsByName("hidden_idmenu")[0];
        var _idmenu = _e_menu.value;
        if(!_idmenu){
...
function removecatemenu(element){
    ...
    var _trash = _e_parent.getElementsByClassName("trash")[0];
     _trash.setAttribute('value', 1);
...
function re_order_block(){
    var arr_categories = [];
    var e_frm_menuhascate = document.getElementsByClassName("frm_menuhascate")[0];
    var _e_list_item = e_frm_menuhascate.getElementsByClassName("item");
    var _idmenu = e_frm_menuhascate.getElementsByClassName("hidden_idmenu")[0];
    //var _idmenu = _e_menu.options[_e_menu.selectedIndex].value;
    document.getElementsByClassName("frm_menuhascate")[0].getElementsByClassName("hidden_idmenu")
    if(!_idmenu){
      alert("Bạn chưa chọn menu");
      return false;
    }
