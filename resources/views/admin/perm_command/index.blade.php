...
@section('content')
   <div class="row">
	...
	<div class="card">
	    <div class="card-body">
	        <h4 class="card-title">Lệnh thực thi</h4>
	        @if($message = Session::get('success'))
	        	<h6 class="card-subtitle">{{ $message }}</h6>
			@endif
			<div align="right">
				<a class="btn btn-default btn-primary" href="{{ route('admin.perm_command.create') }}">Thêm mới</a>
			</div>
	                ...
	                <tbody>
	                	@foreach($perm_commands as $row)
							<tr>
								...
								<td class="btn-control"><a class="btn btn-primary btn-edit" href="{{ action('Admin\perm_commandContronler@edit',$row['idpercommand']) }}"><i class="fa fa-edit"></i></a></td>
								<td class="btn-control">
								     <form method="post" class="delete_form" action="{{action('Admin\perm_commandContronler@destroy', $row['idpercommand'])}}">
								      {{csrf_field()}}
