   ...
   <div class="row">
	...
	<div class="card">
	    <div class="card-body">
			...
			@endif
			<div align="right">
				<a class="btn btn-default btn-primary" href="{{ route('admin.cattype.create') }}">Thêm mới</a>
			</div>
	                ...
	                <tbody>
	                	@foreach($cattypes as $row)
							<tr>
								...
								<td class="btn-control"><a class="btn btn-primary btn-edit" href="{{ action('Admin\CategoryTypeController@edit',$row['idcattype']) }}"><i class="fa fa-edit"></i></a></td>
								<td class="btn-control">
								     <form method="post" class="delete_form" action="{{action('Admin\CategoryTypeController@destroy', $row['idcattype'])}}">
								      {{csrf_field()}}
...
@section('other_scripts')
   <!-- Datatables -->
    ...
    <script src="{{ asset('gentelella/build/js/custom-build.js?v=0.1.3') }}"></script> 
     ...
     <!--<script src="{{ asset('dashboard/production/js/filter_select_category.js?v=0.2.1') }}"></script> 
