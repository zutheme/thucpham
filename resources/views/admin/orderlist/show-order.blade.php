@extends('admin.general',compact('rs_latestproduct'))
                  ...
                  <div class="x_content">
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive" cellspacing="0" width="100%">
                      <thead>
                          ...
                          <tbody>
                            @if(isset($rs_orderproduct))
                              @foreach($rs_orderproduct as $row)
                                    ...
                                    <tr>                     
                                      <td style="text-align: center;"><a href="{{ url('/') }}/{{ $row['slug'] }}"><img width="100%" class="thumb" src="{{ asset($row['urlfile']) }}"></a></td>
						...
						@if($customer['cus'] == 1)
						<tr><td>Thông tin khách hàng</td><td></td><td></td><td></td><td></td><td></td></tr>
						 <tr>
							<td>Họ tên:</td><td>{{ $customer['lastname'] }} {{ $customer['middlename'] }} {{$customer['firstname']}}</td><td></td><td></td><td></td><td></td>
						  </tr>
						  <tr>                
							<td>Điện thoại:</td><td>{{ $customer['mobile'] }}</td><td></td><td></td><td></td><td></td>
						  </tr>
						  <tr>
