   ...
   <div class="row">
   	<div class="x_content">
	        <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                <div align="right">
						<a class="btn btn-default btn-primary" href="{{ URL::route('admin.impperm.create') }}">Thêm mới</a>
					</div>
                  <div class="x_title">
                  ...
                  <div class="x_content">
                   <table id="datatable" class="table table-striped table-bordered">
	                ...
	                <tbody>
	                	@foreach($impperms as $row)
							<tr>
								<td>{{ $row['nameperm'] }}</td>
								<td>{{ $row['namerole'] }}</td>
								<td>{{ $row['nameuser'] }}</td>
								<td class="btn-control"><a class="btn btn-primary btn-edit" href="{{ action('Admin\ImpPermController@edit',$row['idimp_perm']) }}"><i class="fa fa-edit"></i></a></td>
								<td class="btn-control">
								     <form method="post" class="delete_form" action="{{action('Admin\ImpPermController@destroy', $row['idimp_perm'])}}">
								      {{csrf_field()}}
