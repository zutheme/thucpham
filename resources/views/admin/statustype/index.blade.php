...
@section('content')
   <div class="row">
	...
	<div class="card">
	    <div class="card-body">
	        <h4 class="card-title">Kiểu trạng thái</h4>
	        @if($message = Session::get('success'))
	        	<h6 class="card-subtitle">{{ $message }}</h6>
			@endif
			<div align="right">
				<a class="btn btn-default btn-primary" href="{{ route('admin.statustype.create') }}">Thêm mới</a>
			</div>
	                ...
	                <tbody>
	                	@foreach($statustypes as $row)
							<tr>
								...
								<td class="btn-control"><a class="btn btn-primary btn-edit" href="{{ action('Admin\StatusTypeController@edit',$row['id_status_type']) }}"><i class="fa fa-edit"></i></a></td>
								<td class="btn-control">
								     <form method="post" class="delete_form" action="{{action('Admin\StatusTypeController@destroy', $row['id_status_type'])}}">
								      {{csrf_field()}}
